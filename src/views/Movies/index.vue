    ...
    <div class="p-4">
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <input type="text" v-model="searchYear" placeholder="Ano (ex: 2023)" @keyup.enter="handleSearch"
                class="shadow appearance-none border rounded py-2 px-3 text-cineflix-white bg-cineflix-dark-gray leading-tight focus:outline-none focus:shadow-outline w-32" />
        ...
        <div v-if="movieStore.getMovies.length > 0"
            ...
            <div v-for="movie in movieStore.getMovies" :key="movie.imdbID"
                ...
                <img :src="movie.Poster !== 'N/A' ? movie.Poster : 'https://placehold.co/300x450?text=Poster+N/A'"
                    alt="Pôster do Filme" class="w-full h-auto object-cover"
                    @error="e => (e.target as HTMLImageElement).src = 'https://placehold.co/300x450?text=Poster+N/A'" />
                <div class="p-3">
...
onMounted(() => {
    if (movieStore.lastSearchTitle) {
        ...
        movieStore.fetchMovies('Matrix', '1999');
    }
});
